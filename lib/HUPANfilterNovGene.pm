...
#check the binary file
    ...
    die("Executable filterNovelgene.pl cannot be found in your PATH!\n") unless($fpflag);

    my $com="$exefg $seq_file $gff_file $tra_file $pro_file $genome_ref $tran_ref $cdhit_exe $blast_exe $mkblastdb_exe $repeat_exe $gene_identity $ref_identity $thread_num $out_dir\n";
    system($com);
}
1;
