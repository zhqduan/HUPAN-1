...
sub simSeq{
    my $usage="\nUsage: hupanSLURM simSeq [commands] ...

hupanSLURM simSeq is used to simulate and plot the total size of novel sequences.
...
##************** Might be modified for different task submission system *******************
        my $job_file=$script_out."/sim$n.slurm";   #script_file
        ...
        print JOB "\#!/bin/bash\n";
        print JOB "\#SBATCH --job-name=sim$n\n";              #job name
...
sub plotNovelSeq{
    ...
    use vars qw($opt_h);
    getopts("h");
    my $usage="\nUsage: hupanSLURM simSeq plotNovelSeq [commands] <data_directory> <out_directory>

hupanSLURM simSeq plotNovelSeq is used to plot the total size of novel sequences from 'simNovelSeq' result.
